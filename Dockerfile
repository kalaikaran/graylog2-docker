...
# Get graylog2 server
RUN wget -O - -o /dev/null http://packages.graylog2.org/releases/graylog2-server/graylog2-server-0.92.3.tgz | tar -xz -C /opt
RUN ln -s /opt/graylog2-server-0.92.3 /opt/graylog2-server
...
RUN the url 
#check for url name with same password
...
RUN chown graylog2:root /opt/graylog2-server-0.92.3 /opt/graylog2-web-interface-0.92.3
# Expose ports
#   - 9000: Web interface
#   - 12201: GELF UDP
#   - 12900: REST API
